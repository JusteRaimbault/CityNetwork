...
\added{To tackle this ontological issue of the object under study, we (i) stay at a high resolution; and (ii) compute field values with a small enough offset to obtain continuous values.} We choose here the mesoscopic scale of a metropolitan center, of an order of one hundred kilometers, first for the relevance of the spatial field computed, and secondly because smaller scales become less relevant for the notion of urban form, whereas larger scales induce a too large variability. At this scale, we can assume that territorial characteristics, for population and network, are locally defined and vary in an approximately continuous way in space. \added{All metropolitan areas do not have this size, but smaller will be smoothed by the offset whereas larger will be reconstructed by the variation of the field in space.} Thus, the construction of fields of morphological indicators will allow \added{us} to endogenously reconstruct territorial entities through the emergent spatial structure of indicators at larger scales. 
...
\added{Although remaining theoretical and stylized in the case of the model, our result can already suggests possible directions for planning policies.}
