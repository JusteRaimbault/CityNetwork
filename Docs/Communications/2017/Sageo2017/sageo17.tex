...
\title{Identification of causalities in spatio-temporal data}
...
\sframe{Spatio-temporal complexity}{
...
\sframe{Complex relations between networks and territories}{
...
\textit{Fieldwork in Pearl River Delta unveils local manifestations of the co-evolution of transportation networks and territories. Source : Author.}

}

\sframe{The myth of structuring effects}{
...
$\rightarrow$ Existence of co-evolutive processes \cite{bretagnolletel00459720}

\medskip

$\rightarrow$ At large scale, existence of structural urban system dynamics \cite{espacegeo2014effets}

\medskip

$\rightarrow$ The question of circular causalities arises at all scales (e.g. metropolitan scale and mobility \cite{cerqueira2017inegalites}) and in various fields (knowledge spillovers and innovation \cite{audretsch1996r})
...
$\rightarrow$ Classical geography already investigated causal links in space \cite{loi1985etude}

\bigskip

$\rightarrow$ \cite{claval1985causalite} : beyond reductionist causality in systemic analysis

\bigskip

$\rightarrow$ Systemogenesis introduced by \cite{durand2003geographes} focuses on dynamics and path-dependency

\bigskip

$\rightarrow$ Towards a complex approach to causality ? \cite{morin1976methode}
...
\sframe{Existing approaches in spatio-temporal causality}{
...
\textbf{Transportation Networks and Territories}

\begin{itemize}
	\item\justify Lagged correlations : \cite{levinson2008density} London population and network connectivity ; \cite{doi10.1068/b39089} historical data in North Italy
\end{itemize}
\begin{itemize}
	\item\justify Instrumental variables : \cite{duranton2012urban} US highways and employment ; \cite{berger2017locomotives} clear effect of swedish railway on cities trajectories
\end{itemize}
...
\textbf{Spatio-temporal correlations}

\begin{itemize}
	\item Matching method for traffic flows \cite{liu2011discovering}
	\item Generalized granger causality in neuroscience \cite{ke2007spatio}
	\item Spatio-temporal correlations in Computer Vision \cite{ke2007spatio}
\end{itemize}
...
$\rightarrow$ Genericity and operationality of existing approaches ?

\medskip

$\rightarrow$ Grasp complexity in a simple way ?

\medskip

$\rightarrow$ At the interface of knowledge domains (methodology, modeling and empirical) \cite{2017arXiv170609244R}
...
\textit{Explore a generic method based on patterns of spatio-temporal lagged correlations : notion of Granger causality ; validation on synthetic data and application to a case study.}
...
Correlation estimator $\hat{\rho}$ applying in time, space and repetitions, i.e. covariance is estimated by $\hat{\Cov}\left[X,Y\right] = \hat{\mathbb{E}}_{i,t,k}\left[XY\right] - \hat{\mathbb{E}}_{i,t,k}\left[X\right]\hat{\mathbb{E}}_{i,t,k}\left[Y\right]$
...
Patterns of $\textrm{argmax}_{\tau} \rho_{\tau}\left[X_{j_1},X_{j_2}\right]$ or $\textrm{argmin}_{\tau} \rho_{\tau}\left[X_{j_1},X_{j_2}\right]$ (assumed clearly defined : e.g. statistical significance, minimal value) capture the sense of causality between $j_1$ and $j_2$

\medskip

$\rightarrow$ Datamining on $\rho_\tau$ (possibly parametrized values as $\rho_\tau^{(\omega)}$) to understand causality patterns.
...
\footnotesize\textit{Unsupervised classification (robust k-means) on $\tau_{min},\tau_{max}$ features: (Left) Derivative of clustering coefficient for number of clusters $k$; (Right) PCA visualisation of classification for ``optimal'' $k$}
...
$\rightarrow$ Lagged correlation patterns on real data to investigate ``structuring effects'' in complex systems

\medskip

$\rightarrow$ The operational concept of \textit{Causality Regimes} introduces a novel way to look at co-evolution in models of simulation
