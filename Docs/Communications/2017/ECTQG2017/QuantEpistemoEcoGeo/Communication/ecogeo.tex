...
$\rightarrow$ Banos' vertuous circle~\cite{banos2013pour} ; Complex Systems integrated roadmap~\cite{2009arXiv0907.2221B}
...
Example : \cite{fujita1999evolution} an economic geography approach with equilibrium reproducing observable hierarchy ; incompatible with dynamical complex approaches from Evolutive Urban Theory ?~\cite{pumain2012multi}

\bigskip

$\rightarrow$ \textit{Are incompatibilities intrinsic to epistemologies, or how to make bridges ?}
...
\textbf{Research Objective : } In the case of Economics and Geography, how can the way they study specific objects help understand the persistent isolation between the two ?

\bigskip

$\rightarrow$ Quantitative Epistemology approach : investigate ``scientific landscape'' around intersecting ontologies

\medskip

$\rightarrow$ Case studies : Interactions between Networks and Territories ; Urban Growth
...
\begin{itemize}
	\item Hypernetwork approach to the scientific landscape : combination of citation layer with semantic layer~\cite{raimbault2016indirect} \cite{bergeaud2017classifying}
	\item From initial corpus, backward reconstruction of the citation network at depth 2
	\item Collection of abstracts and text-mining to extract relevant keywords (multi-stems)
	\item Intra and interlayer analysis : construction of endogenous domains by community detection ; measures of interdisciplinarity between layers
\end{itemize}
...
\includegraphics[height=0.7\textheight]{figures/fieldwork}
...
\includegraphics[width=\textwidth]{figures/interdisciplinarities}
...
\sframe{Interdisciplinarity patterns}{
\centering
\includegraphics[width=\textwidth]{figures/compo_proportion}
...
Very broad final corpus, from technical GIS studies to entrepreneurship and ``creative clusters'' studies
...
\begin{itemize}
	\item $(V=52560,E=168813)$, modularity 0.70 (null model $2.20\cdot 10^{-5}\pm 0.00078$)
	\item Main communities : Urban Ecology, Urban Sociology, Housing Market, Spatial Statistics, Economic Geography, Criminology, Cellular Automata, Urban Simulation, Development, Ecology, Mobility, LUTI, Networks, Economy of Information and innovation.
\end{itemize}
...
$\rightarrow$ Highly clustered citation practices, but presence of bridges (connected network)

\medskip

$\rightarrow$ Correspondence between citation and semantic communities, but not fully correlated

\medskip

$\rightarrow$ Importance of intermediate fields or sub-disciplines, such as recent disciplines to connect disciplines : Physicists as a way forward ? (\cite{barthelemy2016structure}, \cite{west2017scale}) 
...
$\rightarrow$ Each discipline has its own objects, epistemologies and ontologies : open question on how to identify intersecting ontologies and achieve ``Applied Perspectivism''

$\rightarrow$ Crucial role of intermediate fields to shape research subjects and convergences ? In particular performative branches, that can bring common objects and perspectives

\bigskip

\textbf{Developments}

$\rightarrow$ Towards a scaled and more systematic overview

$\rightarrow$ Trade-offs between horizontal and vertical research : towards an agent-based investigation~\cite{chavalarias2017s}
...
$\rightarrow$ Hypothesis : bridging must require a high level of reflexivity, as the construction of integrative disciplines and approaches to Complex Systems (question : related to the nature of Complexity ?)

\bigskip

$\rightarrow$ Systematic reviews and Quantitative epistemology soon as a necessary step in research ? Empowering open initiatives such as \textit{CybergeoNetworks}\\
\texttt{http://shiny.parisgeo.cnrs.fr/CybergeoNetworks}
...
\\\texttt{QuantEpistemo/HyperNetwork}
}
...
\sframe{Initial Corpuses}{


\textbf{Network and Territories : } Corpus from state of the art and modelography of Modeling the relations between Networks and Territories \cite{juste_raimbault_2017_838263}

\bigskip

\textbf{Urban Growth : } 20 first results of query ``Urban Growth'' in scholar, filtered and balanced subjectively

}

\sframe{Semantic Network Construction}{

Multi-objective optimization between modularity and network size, $\theta_w = 10$, $k_{max} = 500$, compromise point yields $(V = 7063, E = 48952)$

	\centering
	
	\bigskip

\includegraphics[width=0.5\textwidth]{figures/pareto-com-vertices}
\includegraphics[width=0.5\textwidth]{figures/pareto-modularity-vertices}
