...
\item \cite{gleyze2005vulnerabilite} : urban network analyses, concludes that ``limited by computation''

$\rightarrow$ 10 years later : \cite{2015arXiv151201268L} !

\medskip

\item First Simpop models~\cite{sanders1997simpop} ``calibrated'' by hand
...
\item Space syntax : from the theoretical origins~\cite{hillier1989social} to large-scale applications~\cite{hillier2016fourth}
...
\begin{itemize}
\item Mobility studied through various type of data : new data from transportation systems \cite{}% ex citybikes
...
\item Opening of ``classic'' dataset will allow ever more meta-analyses % ex city dock


\bigskip

\item New ways to do research, more interactive, crowd-sourced science and data ? \cite{2016arXiv160606162C} ; \cite{cybergeo20} % CybergeoNetworks and MetaZipf
...
$\rightarrow$ still explorations to do : variable correlations areas ; same work on cities population/train network data, which are also dynamical databases : extrapolation of ergodicity parameters ?
...
\textbf{In our case study : } Without theory, would not know which objects/measures to look at ; multi-scale and dynamical nature ; without analytics : no conclusion from empirical analysis
...
\includegraphics[height=\textheight]{figures/hists_GOOD}
}


%%%%%%%%%%%%%%%%%
\sframe{Network Distribution}{
%\includegraphics[]{} % TODO network stat distrib
}
