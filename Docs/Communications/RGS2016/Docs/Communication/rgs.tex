...
2. \textbf{Global non-ergodicity.} Let $X_k$ a partition of space into local areas. We have $<\cdot>_x = \sum_k w_k <\cdot>_{x_k} = \sum_k w_k <\cdot>_{\mathcal{T}_k} $. On the other hand, global ergodicity gives $<\cdot>_t = <\cdot>_{\mathcal{T}} = \sum_k w_k <\cdot>_{\mathcal{T}}$. Projecting on each yields $\mathcal{T}=\mathcal{T}_k$, what is absurd.
...
$\rightarrow$ Interdisciplinarity necessary to achieve that.
