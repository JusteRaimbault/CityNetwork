...
\begin{itemize}
	...
	\item Modèles pour éprouver la théorie (validation interne et externe).
\end{itemize}


rejoint partiellement raisons d'Epstein \cite{epstein2008model}


\subsubsection{Generative modeling}{Modélisation générative}

Le \emph{type}\footnote{Dans la perspective fonctionnelle % existe classif modeles par type ? non ? pour ca que Varenne par fonctions ? type difficile a definir en lui-meme ? lien outils / methodes : lié à domaines de connaissance
} de modèles que nous utiliserons
...
Tous nos modèles sont a visee de comprehension, meme quand calibration fine ! developper ca en lien avec fonction des modeles ! : revenir sur Varenne, explication et comprehension.




definition du modele selon le perspectivisme.
...
\cite{perez2016agent} perspectives abm urban systems
...
% detailler un peu la ``cuisine'' : stochasticite, explo elementaire, explo ciblee algos.
...
Le cas échéant, certaines étapes n'ont pas lieu d'être, par exemple l'évaluation de la stochasticité dans le cas d'un modèle déterministe. De même, les étapes prendront plus ou moins d'importance selon la nature de la question posée
