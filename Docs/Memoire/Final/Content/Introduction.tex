...
Des conflits similaires se rencontrent en économie : comme décrit par \cite{marchionni2004geographical}, la discipline de l'économie géographique, traditionnellement proche de la géographie, a fortement critiqué un nouveau courant de pensé nommé \emph{économie géographisée}, \comment{(Arnaud)  New economic geography ?}
 dont le but est la spatialization des techniques économiques classiques. Elles n'ont pas les mêmes desseins et buts, et le conflit apparaît comme un malentendu complet vu d'un oeil extérieur.
}
...
\comment{également un développement sur ``quanti-quali''}
...
\noun{Pumain} rappelle dans~\cite{pumain2003approche} une histoire subjective de l'émergence des paradigmes de la complexité en géographie. La cybernétique a produit des théories des systèmes comme celle utilisée par Forrester.\comment{(Florent) pas dvlpé, difficile à lire}
 Plus tard, le glissement vers les concepts de criticalité auto-organisée et d'auto-organisation en physique ont conduit aux développements correspondants en géographie, comme~\cite{sanders1992systeme} qui témoigne de l'application des concepts de la synergétique aux dynamiques des systèmes urbains. Enfin, les paradigmes actuels des systèmes complexes ont été introduits par plusieurs entrées. Par exemple, la nature fractale de la forme urbaine a été introduite par~\cite{batty1994fractal} \comment[AB]{tu trouveras des références plus anciennes  en biblio dans  http://www.persee.fr/doc/spgeo_0046-2497_1990_num_19_1_2943 } et a eu de nombreuses applications jusqu'à des développements plus récents~\cite{keersmaecker2003using}.
\comment{lier avec les approches de Frankhauser}
\noun{Batty} a aussi introduit les automates cellulaires en modélisation urbaine \comment[AB]{A priori c’est plutôt Tobler. Cf le papier de Couclelis http://journals.sagepub.com/doi/abs/10.1068/a170585 } et propose une synthèse jointe avec les modèles basés agents et les fractales dans~\cite{batty2007cities}. 
\comment{small development on West Bettencourt, scaling and Santa fe school\cite{bettencourt2007growth}}
...
\comment[AB]{on dirait que tu as du mal à assumer ton « absence d’ambition » ;-)}
