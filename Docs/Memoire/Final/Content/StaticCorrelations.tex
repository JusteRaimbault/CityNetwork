...
\item Entropie de la distribution, qui exprime l'uniformité\comment[FL]{et a quoi cela sert ?} de la distribution:
\begin{equation}
...
Les deux premiers indices ne sont pas spatiaux, mais nécessaires pour une bonne qualification des distributions de population\comment[FL]{ce qu'il faudrait demontrer}, et sont complétés par les deux derniers prenant en compte l'espace.
...
\item \comment[FL]{beaucoup d'indicateurs non definis, sans meme une reference pour en trouver la definition}
...
\comment[FL]{mot delicat : est-ce une propriete intrinseque ? quelque chose que tu suggere ? etc. $\rightarrow$ revoir le titre.}
...
\begin{enumerate}
\item L'autocorrelation spatiale locale existe en dessous d'une échelle minimale $l_0$ (en d'autres termes le processus est continu dans l'espace) : pour tout $\vec{x}$ et $t$, on a $\left|\rho_{\norm{\Delta \vec{x}} < l_0}\left[Y_i (\vec{x}+\Delta \vec{x},t), Y_i (\vec{x},t) \right]\right| > 0$.\comment[FL]{une valeur absolue positive\ldots ca n'est pas etonnant}
\item Les processus sont localement paramétrisés\comment[FL]{?} : $Y_i = Y_i\left[\alpha_i\right]$, où $\alpha_i (\vec{x})$ varie à l'échelle $l_{\alpha}$, avec $l_{\alpha} \gg l_0$ et est localement stationnaire\comment[FL]{sens ?} dans l'espace.
