...
\comment[FL]{ta distribution n'est pas centree $\rightarrow$ quelle interpretation tires tu de ce choix ? ; legende ? ; attention aux echelles de couleur ; traduire ; a ce stade le lecteur peut se dire `` pour lui un territoire = 1 carre de 100km de cote $\rightarrow$ a priori il sera sceptique : tu dois donc defendre ta posture avec une particuliere clarté}
...
\bpar{}{
Pour la Chine, pour laquelle une selection d'indicateurs est également donnée en~\ref{app:sec:staticcorrelations}, on observe des variations locales et régionales encore plus marquées, ainsi que par exemple les mega-régions urbaines qui se détachent, correspondant à un régime bien particulier.\comment[FL]{tu n'as pas ammene ce cas d'etude}
}
...
\comment[FL]{faut-il garder les 4 ? traduire et expliquer}
...
Nous avons montré empiriquement la non-stationnarité des interactions entre morphologie de la distribution des population et topologie du réseau routier. Celle-ci suggère que les processus spatio-temporels n'ont pas les mêmes vitesses et réagissent et diffusent différemment.
}
