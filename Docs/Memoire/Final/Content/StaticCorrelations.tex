...
\begin{enumerate}
\item Pente de la loi rang-taille $\gamma$, qui exprime le degré de hiérarchie de la distribution, calculé en ajustant une loi de puissance par Moindres Carrés Ordinaires par $\ln \left( P_{\tilde{i}}/P_0\right) \sim k + \gamma\cdot \ln \left(\tilde{i}/i_0\right)$\comment[FL]{k : ?}[la constante de la regression, dummy dans l'indicateur] où $\tilde{i}$ sont les indices de la distribution triée de manière décroissante. Elle est toujours négative\comment[FL]{ou nulle}, et des valeurs proches de zéro signifient une distribution plate.
...
\item Distance moyenne entre individus, qui capture la concentration\comment[FL]{trop rapide} de la population
\[
\bar{d} = \frac{1}{d_M}\cdot \sum_{i<j} \frac{P_i P_j}{P^2} \cdot d_{ij}
\]
...
\comment{\cite{2017arXiv170902939M} deep learning analysis and typology of worldwide street networks}
}
...
\begin{itemize}
\item \emph{Betweenness Centrality}, moyenne $\bar{bw}$ et hiérarchie $\alpha_{bw}$ \comment[FL]{a expliciter}
