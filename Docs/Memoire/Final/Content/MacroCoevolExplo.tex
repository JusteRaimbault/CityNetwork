...
\subsubsection{Model Description}{Description du Modèle}\comment[AB]{reformuler}
...
\begin{itemize}
  ...
  \item Diversité des trajectoires $\mathcal{D}\left[X_i\right]$, qui capture la diversité de profil des séries temporelles\comment[FL]{confusion possible avec forme urbaine et puis qu'est ce que cela signifie ?}, avec $\tilde{X}_i(t)\in \left[0;1\right]$ les trajectoires mises à l'échelle individuellement,
\[
...
\item Complexité moyenne de la forme des trajectoires $\mathcal{C}\left[X_i\right]$, la ``complexité'' d'une trajectoire étant prise simplement dans notre cas par son nombre de points d'inflexion \comment[AB]{pas trivial a definir ca !}\comment[FL]{ah bon pourquoi est ce simple ?}
...
\]
\comment[AB]{pourquoi pas $\Delta Y(t)$ et $\Delta X (t+\tau)$ $\rightarrow$ ce deviendrait un modele avec anticipation plus que retard mais ca resterait $Y=f(X)$}[c'est deja pris en compte car tau peut etre positif ou negatif]
\end{itemize}
