...
\item Ce corpus est alors inspecté pour les résumés et textes complets si nécessaire, les références étant classifiées et caractérisés selon des critères décrits ci-dessous. On obtient alors un corpus final de taille $N_f = 297$, sur lequel des analyses quantitatives sont possibles.
\end{enumerate}
...
\item Le passage en revue manuel des grand corpus permet de pas louper des ``poids lourds'' qui auraient pu être omis en amont~\cite{lissacksubliminal}. La question 
\end{itemize}

note : this review different from manual, car seulement directement concerné, pas associés comme précurseurs (nw growth)


L'opération de construction permet déjà en elle-même de tirer des observations thématiques :

\begin{itemize}
\item modeling? meta-analyse/ review ? -> pondérer ?
\item un certain nombre de papiers discardés : pratique et applications d'autres modèles
\item domaines annexes non considérés
\item what do we mean by model ? (! contradiction with epistemo fwk ? clarifier) modèle de simulation / numérique.
\item prévalence HSR : politique ? voir selon stats caracs
\end{itemize}
...
\caption[Systematic Review][Revue Systématique]{}{\textbf{Méthodologie de la revue systématique.}}
