...
L'architecture du modèle couple de manière complexe un module pour l'évolution de l'usage du sol à un module de croissance de réseau de transport.
}


%L'accessibilité, définie comme le potentiel d'accès des habitants à l'emploi, est la pierre angulaire de ce modèle, et repose sur l'hypothèse simplificatrice que à la fois les ménages et les entreprises cherchent à se localisation de façon à maximiser leur accessibilité, et que les acteurs décident de la construction de nouvelles infrastructures de transport sur le critère de la maximisation de l'accessibilité sur leur périmètre.

%Concrètement, le modèle LUTECIA repose sur 5 modèles dont 3 seront détaillés vu le cas d'étude : la croissance du réseau du delta de la Rivière des Perles.

\bpar{
\begin{itemize}
\item LU stands for Land Use module : il régit la relocalisation des actifs et des emplois dans les conditions courante d'accessibilité.
\item T stands for Transport module : calcule les conditions de transport dans la région urbaine.
\item EC stands for Evaluation of Cooperation module : évalue le ou les agents qui vont prendre la décision de construire une infrastructure nouvelle. 
\item I stands for Infrastructure provision module : évalue la localisation de l'infrastructure de transport nouvelle, sur le critère de l'augmentation maximale de l'accessibilité
\item A stands for Agglomeration economies module : évalue la productivité des entreprises, qui dépend de l'accessibilité aux emplois.
\end{itemize}
}{
...
Le sous-modèle pour la gouvernance suit la logique suivante :
\begin{itemize}
...
%\item Possible strategies for players (negotiating areas, $i=1,2$) are : not collaborating ($NC$) and collaborating ($C$). Strategies are chosen simultaneously (non-cooperative game) as detailed after. For $(C,NC)$ and $(NC,C)$ couples, the collaborating agent loose its investment and cannot build a road whereas the other continues his business alone. For $(NC,NC)$ both act as alone, and for $(C,C)$ a common development is done. We denote $Z^{\ast}_i(S_1,S_2)$ the optimal infrastructure for area $i$ with $(S_1,S_2)\in \{(NC,C),(C,NC),(NC,NC)\}$ which are determined the standard way in each zone separately, and $Z^{\ast}_C$ the optimal common infrastructure computed with a 2 segments infrastructure on the union of both areas, which corresponds to the case where both strategies are $C$. Marginal accessibilities for area $i$ and infrastructure $Z$ is defined as $\Delta X_i(Z)=X^Z_i - X_i$. We introduce the costs of construction,  noted $I$ for a road segment, assumed spatially uniform. A 2 road segment will cost $2\cdot I - J$ ($J > 0$ cost gain of common technical means, assumed to be equally shared). 
%The determination of probabilities defining mixed strategies is based on the notion of payoff matrix, that is the value of utility gains for each players and each possible strategy configuration. The payoff matrix of the game is the following, with $\kappa$ a normalization constant (``price of accessibility'') :
...
%
%
%We can solve the mixed strategy Nash Equilibrium in all generality, by writing $U_i(S_1,S_2)$ the full payoff matrix. The assumption of equilibrium implies that conditional expectancies of each player are equal given their two choices, i.e. that $\Eb{U_i|S_i=C} = \Eb{U_i|S_i=NC}$. It yields
%%By denoting $\Delta_{\left[j|S\right]} U_i = U_i(S,C) - U_i() $ 
%
%\[
...
%
%
%\paragraph{An alternative discrete choice ``game''}{Choix discrets}
%
...
%\]
%
%This type of game is expected to behave differently and will be studied as a benchmark.
%
%
%
%\begin{table}
%\caption{\textbf{Summary of model parameters with their default values.}}
%\centering
%\hspace{-1cm}
...
%  \hline
%\multirow{4}{*}{Land-use}& $\lambda$ & Accessibility distance decay & Accessibility & $]0;1]$ & $0.001$ \\\cline{2-6}
% & $\gamma_A$ & Actives Utility Cobb-Douglas exponent & Utility & $[0;1]$ & $0.85$ \\\cline{2-6}
%  & $\beta$ & Discrete choice exponent & Utility & $[0;+\infty]$ & $1$ \\\cline{2-6}
% & $\gamma_E$ & Employments Utility Cobb-Douglas exponent & Utility & $[0;1]$ & $0.85$ \\\hline
%\multirow{2}{*}{Transportation} & $v_G$ & Network relative speed & Transport hierarchy & $[1;+\infty [$ & $5$ \\\cline{2-6}
%& $\lambda$ & Accessibility distance decay & Commuting range & $[0;+\infty [$ & $5$ \\
%  \hline
%\multirow{3}{*}{Governance} & $I$ & Construction cost & \multirow{3}{*}{Network Planning} & $$ & $0.001$ \\\cline{2-3}\cline{5-6}
...
\bpar{
A complementary approach is to take a user-centered view and consider the proximity of shortest paths : for example with the Mean-square error between distance matrices on all patches, or the Earth Mover Distance. \comment[FL]{mieux hiérarchiser la structure de ce paragraphe, par exemple la métrique est à décrire plus tard ; sans doute plusieurs points à enlever} 

}{
Une approche complémentaires est de prendre une vue centrée sur l'utilisateur et de considérer la proximité des plus courts chemins : par exemple avec une distance euclidienne entre les matrices de distances sur l'ensemble des patches, ou la distance EMD.
}
...
%
%We work on a simplified raster configuration for Pearl River Delta, and on stylized highway networks. We choose to consider road network only since, following \cite{hou2011transport}, it has been the main driver of changes in accessibility patterns compared to railway network which development is only recent. Furthermore, our model is simple and includes one transportation mode only, represented by the road network.\comment[FL]{lequel? pourquoi? ce n'est pas clair et pas justifié. en lien avec partie "Chine" à écrire qui créera dans l'esprit du lecteur le sens de ce choix}
%
%\comment{include and implement cost difference between bridge - standard freeway $\rightarrow$ additional parameter}
%
%
...
%%\begin{figure}
%%\centering
%%\includegraphics[width=0.49\textwidth]{figures/exrun_2_tick0}
%%\includegraphics[width=0.49\textwidth]{figures/exrun_2_tick6}
%%\caption{Example of run on the stylized Pearl River Delta (initial configuration and after 6 network iterations)}
%%\label{fig:ex-prd}
%%\end{figure}
%%%%%%%%%%%%%%%%%%%%
%
