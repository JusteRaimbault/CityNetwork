...
\sframe{Aperçu des contributions en modélisation}{
	...
	\begin{itemize}
		\item \only<2->{\begin{mdframed}[linecolor=red]}\noindent Modèles d'interaction entre villes incluant le réseau \only<2->{\end{mdframed}}
...
\begin{itemize}
	\item \textit{Définition :} relecture possible de la théorie évolutive, ouvre des ponts vers l'économie géographique
	\item \textit{Caractérisation : } nombreuses perspectives d'application en géographie, sciences territoriales
	\item \textit{Modélisation : } des modèles interdisciplinaires ayant vocation à être couplés et réutilisés
\end{itemize}
