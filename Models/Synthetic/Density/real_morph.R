...
vals = list()
i=1
for(x in seq(from=1,to=nrow(raw)-areasize,by=offset)){
  for(y in seq(from=1,to=ncol(raw)-areasize,by=offset)){
     show(paste(x,y))
     
     e<-getValuesBlock(raw,row=x,nrows=areasize,col=y,ncols=areasize)
     # compute on areas with not too much NAs
     if(sum(is.na(e))/length(e)<0.1){
       ...
       # evaluate total computation time
       # 
       # t_exec = system.time(c(x,y,moranIndex(),averageDistance(),entropy(),rankSizeSlope()))
       #
       # t_tot = (nrow / offset) * (ncol / offset) * t_exec
       #
       # (offset = 5km and aggregation = 0.5) -> 52.4h with full grid, so real time a bit less
       #
...
write.table(
  v,
  file=paste0(Sys.getenv("CN_HOME"),'/Results/Synthetic/Density/RealData/Numeric/france_20km.csv'),
  sep = ";",
