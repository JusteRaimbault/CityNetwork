...
;  let b false
;  ask noeuds[
;     if count my-aretes <= 2 and k < 1e-5 [
;       set b true
;       die
;     ]
;  ]
...
to choose-OD
  ask c1 [ set pcolor white ]
  ask c2 [ set pcolor white ]
  
  let s sum [p] of poles
  let r random s                         ;; 1er pole au hasard
  foreach sort poles [
    ask ? [
      if acc-p <= r and r < acc-p + p [
        set c1 self
        stop
      ]
    ]
  ...
  ask c1 [
    set pcolor blue
    set r random s - p                   ;; 2nd pole au hasard
    if acc-p <= r [set r (r + p)]
    foreach sort other poles [
      ask ? [
        if acc-p <= r and r < acc-p + p [
          set c2 self
          stop
...
to reset-p [Pm] ; noeuds procedure
...
ask aretes with [D > Df] [set color red]
NIL
1
T
OBSERVER
