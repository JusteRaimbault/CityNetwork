...
ls -d -1 $PWD/calibration/**/* > dirs

# create target dirs
cat dirs | awk -F"/" '{print "mkdir calibres/'$TARGET'/"$NF}' | sh
...
while read p; do
  ...
  DIR="${array[-1]}"
  echo $DIR
  ls $p | awk -F"." '{print $1}' | awk -F"n" '{print $2}'|sort -n | tail -n 1 | awk '{print "cp '$p'/population"$0".csv calibres/'$TARGET'/'$DIR'"}' | sh
done < dirs
