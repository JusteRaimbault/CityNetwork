...
# drop old base
psql -p 5433 -d postgres -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '"$DB"';"
psql -p 5433 -d postgres -c "DROP DATABASE "$DB";"
# recreates it
psql -p 5433 -d postgres -c "CREATE DATABASE "$DB";"
...
osmosis --read-pbf $FILE --log-progress --tf accept-ways highway=* --used-node --write-pgsql host=localhost:5433 database=$DB
